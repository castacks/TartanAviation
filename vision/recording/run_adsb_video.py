...
import csv
import json
from imx_cameras import IMXCameraLogging
...
class DataRecord:
    ...
    def __init__(self):
        ...
        self.camera_recorder = IMXCameraLogging()
    ...
    def startWriting(self):
        now = datetime.now()
        self.filepath = '/data/' + now.strftime("%Y-%m-%d")
        if not os.path.exists(self.filepath):
            os.makedirs(self.filepath)
        if (self.recording is False):
            # self.filename = getNextFilePath(self.filepath,'.txt')
            self.filename = os.path.join(self.filepath, now.strftime("%H-%M-%S"))
