...
if args.option == 'Date_Range' or args.option == 'All':
    ...
    for location in locations:
        ...
        for year in range(int(start_date.split('-')[0]), int(end_date.split('-')[0])+1):
            for month in range(int(start_date.split('-')[1]), int(end_date.split('-')[1])+1):
                # Create the date string
                date = f'{year}/{month:02d}'
                # Create the path to download
                for day in range(1, 32):
                    audio_path = f'{location}/{date}/{month:02d}-{day:02d}-{str(year)[-2:]}_audio'
